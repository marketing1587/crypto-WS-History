...
mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitfinex_historique_5min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_5min = database.db("bitfinex_historique_5min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitfinex_historique_10min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_10min = database.db("bitfinex_historique_10min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitfinex_historique_15min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_15min = database.db("bitfinex_historique_15min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitfinex_historique_30min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_30min = database.db("bitfinex_historique_30min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitfinex_historique_1h', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_1h = database.db("bitfinex_historique_1h")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitfinex_historique_4h', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_4h = database.db("bitfinex_historique_4h")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitfinex_historique_24h', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_24h = database.db("bitfinex_historique_24h")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds123500.mlab.com:23500/bitfinex_historique', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db = database.db("bitfinex_historique")
    console.log("in bitfinex_historique")

    var arr=[] 

    db_pairs.collection("BITFINEX_PAIRS") 
        .find()
        .toArray(function (err, doc) {
            var my_pair
            for (var i = 0; i < doc.length; i++) {
                my_pair_dbformat = doc[i].name
                my_pair_wsformat = "t" + doc[i].name.replace("_","")
                arr.push({'my_pair_wsformat':my_pair_wsformat, 'my_pair_dbformat': my_pair_dbformat })
            }
            ...
            for (var i=0; i<arr.length; i++) {
                let msg_send = JSON.stringify({
                    event: 'subscribe',
                    channel: 'ticker',
                    symbol: arr[i].my_pair_wsformat,
                });
                f(msg_send, i, arr[i]);
            }

            var myobjet = new Object();
            function f(msg_send, i, my_global_pair) {

                channels[i] = new ws('wss://api.bitfinex.com/ws/2', { version: 2, autoOpen: false })
                channels[i].on('open', () => channels[i].send(msg_send))
                channels[i].on('message', (msg_received) => {
 
                    if ((msg_received.indexOf("event") == -1) && (msg_received.indexOf("hb") == -1)) {
                        
                            temp_msg_received =(JSON.parse(msg_received))[1];
                            my_pair = my_global_pair.my_pair_dbformat
                            var my_change = (Number(temp_msg_received[5]) * 100).toFixed(2);
                            var mydate = getMyDate(new Date());
                            myobjet[my_pair] = {exchange: "BITFINEX", price: temp_msg_received[6], change: my_change, volume: temp_msg_received[7], high :temp_msg_received[8], low: temp_msg_received[9], date: mydate }
                    }
               })
            }
            //1 min
            ...
            setInterval(function () {
               for (property in myobjet) {
                ...
                i++;
                console.log("BIF  i " + i + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 1);
            ...
            setInterval(function () {
               for (property in myobjet) {
                ...
                i_5++;
                console.log("BIF  i_5 " + i_5 + " date: " + my_historique_date.split(" ")[1])
            },1000 * 60 * 5);
            ...
            setInterval(function () {
               for (property in myobjet) {
                ...
                i_10++;
                console.log("BIF  i_10 " + i_10 + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 10); 
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_15++;
                 console.log("BIF  i_15 " + i_15 + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 15);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_30++;
                 console.log("BIF  i_30 " + i_30 + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 30);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_1h++;
                 console.log("BIF  i_1h " + i_1h + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 60);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_4h++;
                 console.log("BIF  i_4h " + i_4h + " date: " + my_historique_date.split(" ")[1])
             }, 1000 * 60 * 60 * 4);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_24h++;
                 console.log("BIF  i_24h " + i_24h + " date: " + my_historique_date.split(" ")[1])
             }, 1000 * 60 * 60 * 24); 
        })
})
