...
mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitstamp_historique_5min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_5min = database.db("bitstamp_historique_5min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitstamp_historique_10min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_10min = database.db("bitstamp_historique_10min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitstamp_historique_15min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_15min = database.db("bitstamp_historique_15min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitstamp_historique_30min', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_30min = database.db("bitstamp_historique_30min")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitstamp_historique_1h', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_1h = database.db("bitstamp_historique_1h")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitstamp_historique_4h', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_4h = database.db("bitstamp_historique_4h")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds129560.mlab.com:29560/bitstamp_historique_24h', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
    db_24h = database.db("bitstamp_historique_24h")
})

mongodb.MongoClient.connect('mongodb://ortal:Ortal1234@ds123500.mlab.com:23500/bitstamp_historique', function (err, database) {
    if (err) {
        console.log(err);
        process.exit(1);
    }
  
    db = database.db("bitstamp_historique")
    console.log("in bitstamp_historique")

    var arr=[] 

    db_pairs.collection("BITSTAMP_PAIRS")
        .find()
        .toArray(function (err, doc) {
            var my_pair
            for (var i = 0; i < doc.length; i++) {
                my_pair_dbformat = doc[i].name
                my_pair_wsformat = doc[i].name.split("_")
                my_pair_wsformat = (my_pair_wsformat[0] + my_pair_wsformat[1]).toLowerCase()
                arr.push({'my_pair_wsformat':my_pair_wsformat, 'my_pair_dbformat': my_pair_dbformat })
            }
   
            var channels = []
            var live_trade = 'live_trades_'

            for (var i = 0; i < arr.length; i++) {
                if(arr[i].my_pair_wsformat == 'btcusd') { var live_trade_pair = 'live_trades'}
                else {var live_trade_pair = live_trade + arr[i].my_pair_wsformat}
                f(live_trade_pair, i, arr[i].my_pair_dbformat)
            }

            var myobjet = new Object();
            function f(live_trade_pair, i, pair) {

                var pusher = new Pusher('de504dc5763aeef9ff52');
                channels[i] = pusher.subscribe(live_trade_pair)

                channels[i].bind('trade', function (data) {
                    var mydate = getMyDate(new Date());
                    myobjet[pair] = {exchange: "BITSTAMP", price: data.price, date:mydate}
                }) 
            }
            ...
            setInterval(function () {
               for (property in myobjet) {
                ...
                i++;
                console.log("BIS  i " + i + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 1);
            ...
            setInterval(function () {
               for (property in myobjet) {
                ...
                i_5++;
                console.log("BIS  i_5 " + i_5 + " date: " + my_historique_date.split(" ")[1])
            },1000 * 60 * 5);
            ...
            setInterval(function () {
               for (property in myobjet) {
                ...
                i_10++;
                console.log("BIS  i_10 " + i_10 + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 10); 
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_15++;
                 console.log("BIS  i_15 " + i_15 + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 15);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_30++;
                 console.log("BIS  i_30 " + i_30 + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 30);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_1h++;
                 console.log("BIS  i_1h " + i_1h + " date: " + my_historique_date.split(" ")[1])
            }, 1000 * 60 * 60);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_4h++;
                 console.log("BIS  i_4h " + i_4h + " date: " + my_historique_date.split(" ")[1])
             }, 1000 * 60 * 60 * 4);  
             ...
             setInterval(function () {
                for (property in myobjet) {
                 ...
                 i_24h++;
                 console.log("BIS  i_24h " + i_24h + " date: " + my_historique_date.split(" ")[1])
             }, 1000 * 60 * 60 * 24); 
        })
})
